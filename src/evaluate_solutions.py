"""
Script to evaluate generated code solutions.
Runs tests and computes compile@1 and pass@1 metrics.
"""
...
def evaluate_solutions(
    ...
    generated_indices = file_manager.get_all_generated_indices()
    ...
    indices_to_evaluate = [
        idx for idx in generated_indices
        if start_idx <= idx < end_idx
    ]
    
    logger.info(f"Evaluating {len(indices_to_evaluate)} solutions in range [{start_idx}, {end_idx})")
    ...
    for idx in tqdm(indices_to_evaluate, desc="Evaluating solutions"):
        try:
            # Load generated code
            code = file_manager.load_code(idx)
            if code is None:
            ...
            # Run all test cases
            all_passed, passed_count, total_count, test_results = code_executor.evaluate_sample(
                code=code,
            ...
            if all_passed:
                ...
                logger.warning(f"Sample {idx}: FAILED - passed {passed_count}/{total_count} test case(s)")
            ...
            for test_result in test_results:
                ...
                if test_result['success']:
                    if test_result['passed']:
                        ...
                        logger.debug(f"Sample {idx}, Test {test_idx}: FAILED (output mismatch)")
                else:
                    logger.debug(f"Sample {idx}, Test {test_idx}: ERROR - {test_result['error']}")
    ...
    # Calculate metrics
    compile_at_1 = (stats['compiled'] / stats['total_samples'] * 100) if stats['total_samples'] > 0 else 0
    pass_at_1 = (stats['passed_all_tests'] / stats['total_samples'] * 100) if stats['total_samples'] > 0 else 0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate generated code solutions"
    )
